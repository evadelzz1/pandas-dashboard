...
html_title = """
    ...
    <center><h1 class="title-head">Adidas Interactive Sales Dashboard</h1></center>
"""
...
view1, dwn1, view2, dwn2 = st.columns([0.35,0.15,0.35,0.15])
with view1:
...
fig3.add_trace(go.Scatter(x=result2["State"], y = result2["UnitsSold"], mode = "lines", name ="Units Sold", yaxis="y2"))
fig3.update_layout(
...
view4, dwn4, view5, dwn5 = st.columns([0.35,0.15,0.35,0.15])
with view4:
    result3 = df[["Region","City","TotalSales"]].groupby(by=["Region","City"])["TotalSales"].sum()
    expander = st.expander("View data for Total Sales by Region and City")
    expander.write(result3)
    
with dwn4:
    st.download_button("Download Data", data = result3.to_csv().encode("utf-8"),
